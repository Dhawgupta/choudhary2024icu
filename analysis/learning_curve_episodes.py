...
from analysis.utils import find_best, smoothen_runs, find_best_episode
...
for en, js in enumerate(json_handles):
    run, param , data = find_best_episode(js, data = key_to_plot, metric = metric, minmax = 'max')
    print(param)
    agent = param['algo']    
...
plt.savefig(f'{foldername}/learning_curve_{get_experiment_name}_episodes.png', dpi = 300)
